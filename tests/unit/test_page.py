...
class TestPageCollectionInit:
    ...
    def test_init_with_site_and_pages(
        self, mock_site_no_http: Site, mock_page_no_http: Page
    ) -> None:
        """サイトとページリストで初期化できる"""
...
class TestPageCollectionParse:
    ...
    def test_parse_empty_result(
        self, mock_site_no_http: Site, page_listpages_empty: dict[str, Any]
    ) -> None:
        """空結果をパースできる"""
    ...
    def test_parse_no_element_exception(
        self, mock_site_no_http: Site, page_listpages_invalid: dict[str, Any]
    ) -> None:
...
class TestPageProperties:
    ...
    def test_source_property_auto_acquire(
        self, mock_page_with_id: Page, page_viewsource: dict[str, Any]
    ) -> None:
        """ソースが未取得の場合自動取得する"""
        mock_response = MagicMock()
        mock_response.json.return_value = page_viewsource
        mock_page_with_id.site.amc_request = MagicMock(return_value=[mock_response])  # type: ignore[method-assign]

        source = mock_page_with_id.source
        assert "page content" in source.wiki_text

    def test_revisions_property(
        self, mock_page_with_id: Page, page_revisionlist: dict[str, Any]
    ) -> None:
        """リビジョンプロパティが正しく動作する"""
        mock_response = MagicMock()
        mock_response.json.return_value = page_revisionlist
...
class TestPageWriteMethods:
    ...
    def test_destroy_success(
        ...
        mock_page_with_id.site.client.login_check = MagicMock()  # type: ignore[method-assign]
    ...
    def test_commit_tags_success(
        self, mock_page_with_id: Page, page_savetags_success: dict[str, Any]
    ) -> None:
        """タグを正常に保存できる"""
        mock_response = MagicMock()
...
class TestPageCreateOrEdit:
    ...
    def test_create_new_page(
        ...
        mock_site_no_http.client.login_check = MagicMock()  # type: ignore[method-assign]
        ...
        mock_site_no_http.amc_request = MagicMock(  # type: ignore[method-assign]
            side_effect=[
                [mock_lock_response],
                [mock_save_response],
    ...
    def test_edit_locked_page(
        self, mock_site_no_http: Site, page_pageedit_locked: dict[str, Any]
    ) -> None:
        """ロック済みページの編集で例外"""
        mock_site_no_http.client.is_logged_in = True
        mock_site_no_http.client.login_check = MagicMock()  # type: ignore[method-assign]
    ...
    def test_edit_raise_on_exists(self, mock_site_no_http: Site) -> None:
        ...
        with pytest.raises(exceptions.TargetExistsException):
            Page.create_or_edit(
                mock_site_no_http, "existing-page", raise_on_exists=True
            )
...
class TestPageEdit:
    ...
    def test_edit_existing_page(
        ...
        mock_page_with_id.site.amc_request = MagicMock(  # type: ignore[method-assign]
            side_effect=[

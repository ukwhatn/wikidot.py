...
class TestForumThreadCollectionParseListInCategory:
    ...
    def test_parse_success(
        ...
        collection = ForumThreadCollection._parse_list_in_category(
            mock_site_no_http, html
        )
    ...
    def test_parse_with_category(
        ...
        html = BeautifulSoup(forum_threads_in_category["body"], "lxml")
        collection = ForumThreadCollection._parse_list_in_category(
            mock_site_no_http, html, mock_forum_category_no_http
        )
...
class TestForumThreadCollectionAcquireAll:
    ...
    def test_acquire_all_single_page(
        ...
        mock_forum_category_no_http.site.amc_request = MagicMock(return_value=[mock_response])  # type: ignore[method-assign]

        collection = ForumThreadCollection.acquire_all_in_category(
    ...
    def test_acquire_all_pagination(
        ...
        mock_forum_category_no_http.site.amc_request = MagicMock(side_effect=[[first_response], [second_response]])  # type: ignore[method-assign]
...
class TestForumThreadCollectionAcquireFromIds:
    ...
    def test_acquire_from_ids_success(
        self, mock_site_no_http: Site, forum_thread_detail: dict[str, Any]
    ) -> None:
        ...
        collection = ForumThreadCollection.acquire_from_thread_ids(
            mock_site_no_http, [3001]
        )
        assert len(collection) == 1
...
class TestForumThreadPosts:
    ...
    def test_posts_property_calls_acquire(
...
class TestForumThreadReply:
    ...
    def test_reply_not_logged_in(
        self, mock_forum_thread_no_http: ForumThread
    ) -> None:
        """ログインしていない場合に例外"""
        mock_forum_thread_no_http.site.client.is_logged_in = False
        mock_forum_thread_no_http.site.client.login_check = MagicMock(  # type: ignore[method-assign]
